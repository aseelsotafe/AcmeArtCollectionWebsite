...
try {
    // Check if style ID provided in URL
    if(isset($_GET['style_id'])) {
        $style_id = $_GET['style_id'];
        
        // Prepare and execute SQL query to get paintings with specified style
        $sql = "SELECT p.*, a.Name AS Artist, s.Name AS Style 
                ...
                WHERE p.StyleID = :style_id";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':style_id', $style_id, PDO::PARAM_INT);
        $stmt->execute();
    } else {
        // If no style ID provided, get all paintings
...
<div class="container-fluid" style="padding-top: 80px; padding-left: 15px; padding-right: 15px; padding-bottom: 50px">
    ...
    <table class="table table-striped" style="width: 49%; margin: 0 auto">
    <thead>
        <tr>
            <th style="width: 5%;"></th>            
